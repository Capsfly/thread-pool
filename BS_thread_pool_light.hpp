...
public:
    ...
    /**
     * @brief Destruct the thread pool. Waits for all tasks_to_be_executed to complete, then destroys all threads_ptr.
     */
    ~thread_pool_light()
    {
        wait_for_tasks();
    ...
    /**
     * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue. The user must use wait_for_tasks_done() or some other method to ensure that the loop finishes executing, otherwise bad things will happen.
     *
    ...
    /**
     * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue. The user must use wait_for_tasks_done() or some other method to ensure that the loop finishes executing, otherwise bad things will happen. This overload is used for the special case where the first index is 0.
     *
     * @tparam F The type of the function to loop through.
    ...
    /**
     * @brief Wait for tasks_to_be_executed to be completed. Normally, this function waits for all tasks_to_be_executed, both those that are currently is_running in the threads_ptr and those that are still waiting in the queue. Note: To wait for just one specific task, use submit() instead, and call the wait() member function of the generated future.
     */
    void wait_for_tasks()
    {
        waiting = true;
